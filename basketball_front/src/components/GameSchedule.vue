    ...
    <div class="game-schedule-page">
        ...
        <table class="game-schedule-table">
            ...
            <tr v-for="game in games.records" :key="game.id">
                ...
                <td>
                    <el-button v-if="game.state === 2" @click="fetchPlayerStats(game.id)" class="analysis-button" size="mini">数据分析</el-button>
                    <el-button v-if="game.state === 2" @click="openScoreModal(game.id)" class="score-button" size="mini">我要评分</el-button>
                </td>
...
export default {
    ...
    methods: {
        fetchGameSchedule() {
            ...
            axios.get('http://192.168.43.201:8088/gameSchedule', {  headers: {
                    'Authorization': token
                },params })
                .then(response => {
        ...
        openAnalysis(gameId) {
            // Placeholder for opening game analysis
            console.log("Open analysis for game:", gameId);
        },
        openScore(gameId) {
            // Placeholder for opening game analysis
            console.log("Open score for game:", gameId);
        },gameState(state) {
            const stateMap = {
                0: '未开始',
                1: '进行中',
